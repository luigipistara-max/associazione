                ...
                if ($memberId) {
                    ...
                    $stmt = $pdo->prepare("
                        UPDATE " . table('members') . " SET
                            ...
                            birth_date = ?, birth_place = ?, birth_province = ?, gender = ?, email = ?, phone = ?,
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" name="membership_number" id="membership_number" class="form-control" value="<?php echo e($member['membership_number']); ?>">
                        <button type="button" class="btn btn-outline-secondary" onclick="suggestMembershipNumber()" title="Suggerisci numero tessera">
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" name="fiscal_code" id="fiscal_code" class="form-control text-uppercase" 
                               value="<?php echo e($member['fiscal_code']); ?>" 
                               maxlength="16" pattern="[A-Za-z0-9]{16}" required>
                        <button type="button" class="btn btn-outline-secondary" onclick="calculateFiscalCode()" title="Calcola Codice Fiscale">
                ...
                <div class="col-md-2 mb-3">
                    ...
                    <select name="gender" id="gender" class="form-select">
                        ...
                        <option value="M" <?php echo ($member['gender'] ?? '') === 'M' ? 'selected' : ''; ?>>Maschio</option>
                        <option value="F" <?php echo ($member['gender'] ?? '') === 'F' ? 'selected' : ''; ?>>Femmina</option>
                    </select>
                </div>
                <div class="col-md-3 mb-3">
                    <label class="form-label">Data di Nascita</label>
                    <input type="date" name="birth_date" id="birth_date" class="form-control" value="<?php echo e($member['birth_date']); ?>">
                </div>
                <div class="col-md-2 mb-3">
                    <label class="form-label">Luogo di Nascita</label>
                    <input type="text" name="birth_place" id="birth_place" class="form-control" value="<?php echo e($member['birth_place']); ?>">
                </div>
...
<script>
function suggestMembershipNumber() {
    fetch('<?php echo h($basePath); ?>api/get_next_membership_number.php')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                document.getElementById('membership_number').value = data.number;
            } else {
                alert('Errore nel suggerimento del numero tessera');
            }
        })
        .catch(error => {
...
function calculateFiscalCode() {
    const lastName = document.getElementById('last_name').value.toUpperCase();
    const firstName = document.getElementById('first_name').value.toUpperCase();
    const birthDate = document.getElementById('birth_date').value;
    const birthPlace = document.getElementById('birth_place').value.toUpperCase();
    const gender = document.getElementById('gender').value;
    
    if (!lastName || !firstName || !birthDate || !birthPlace || !gender) {
        alert('Compila tutti i campi richiesti: Cognome, Nome, Data di nascita, Luogo di nascita e Sesso');
        return;
    }
    
    // Chiama API per calcolare CF
    fetch('<?php echo h($basePath); ?>api/calculate_fiscal_code.php', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.error || 'Errore nel calcolo del codice fiscale');
        }
    })
    .catch(error => {
        console.error('Error:', error);
